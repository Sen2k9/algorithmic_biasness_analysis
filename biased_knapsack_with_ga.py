...
def cal_fitness(weight, value, population, threshold,biased_node):
    fitness = np.empty(population.shape[0])  # creating array of fitness based on number of chromosome
    
    for i in range(population.shape[0]):
        if not population[i][biased_node]:
            #print(population[i][biased_node-1])
            fitness[i] = 0
            continue
...
def optimize(weight, value, population, pop_size, num_generations, threshold,biased_node):
    ...
    for i in range(num_generations):  # for each generation/iteration
        ...
        fitness = cal_fitness(weight, value, population, threshold,biased_node)  # calculate fitness for each population
        ...
        fitness_last_gen = cal_fitness(weight, value, population, threshold,biased_node)
...
def run(n, value, weight, threshold,itr,biased_node):
    ...
    parameters, fitness_history, total_coverage, count = optimize(
        weight, value, initial_population, pop_size, num_generations, knapsack_threshold,biased_node)
